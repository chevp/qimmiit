                        ...
                        ktxTextureCreateStorageEnum storageAllocation)
{
    ...
    super->isCompressed = (This->formatInfo.flags & GL_FORMAT_SIZE_COMPRESSED_BIT);
    ...
    else
    {
        ...
        super->glType = glGetTypeFromInternalFormat(createInfo->glInternalformat);
    ...
    if (createInfo->baseWidth == 0 || createInfo->baseHeight == 0 || createInfo->baseDepth == 0)
        return KTX_INVALID_VALUE;
    ...
    case 3:
         ...
         * OpenGL or Vulkan.
         */
        if (createInfo->isArray || createInfo->numFaces != 1 || createInfo->numLayers != 1)
            return KTX_INVALID_OPERATION;
                                  ...
                                  ktxTextureCreateFlags createFlags)
{
    ...
    assert(This->stream.type == eStreamTypeFile || This->stream.type == eStreamTypeMemory);
    stream = &This->stream;
     ...
     */
    if (result == KTX_SUCCESS && (createFlags & KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT))
    {
 ...
 *                 to be freed.
 */
void ktxTextureInt_destruct(ktxTextureInt *This)
{
    ktxTexture *super = (ktxTexture *)This;
