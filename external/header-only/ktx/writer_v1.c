             ...
             GLuint numImages, KTX_image_info images[])
{
    ...
    else
    {
        ...
        if (expectedFormat == GL_INVALID_VALUE || expectedFormat != header.glFormat)
            return KTX_INVALID_OPERATION;
        expectedType = getTypeFromInternalFormatLegacy(header.glInternalformat);
        if (expectedType == GL_INVALID_VALUE || expectedType != header.glType)
            return KTX_INVALID_OPERATION;
    ...
    for (level = 0, i = 0; level < numMipmapLevels; ++level)
    {
        ...
        packedImageBytes = groupBytes * pixelWidth * pixelHeight;
 ...
 */
void getFormatSizeLegacy(GLenum internalFormat, GlFormatSize *pFormatSize)
{
    switch (internalFormat)
    {
    case GL_LUMINANCE8:
