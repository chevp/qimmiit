...
namespace overlayimgui
{
	...
	UI::UI(oceancore::VulkanDevice *vulkanDevice, VkRenderPass renderPass, VkQueue queue,
		   VkPipelineCache pipelineCache, VkSampleCountFlagBits multiSampleCount)
	{
		...
		*/
		...
		ocean::info("Loading ImGui font from: " + fontPath);
		io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 20.0f);
		ocean::info("ImGui font loaded successfully");
#endif
		...
		*/
		...
		pipelineCI.layout = pipelineLayout;
		shaderStages = {
			oceanutil::loadShader(device, "ui.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
			oceanutil::loadShader(device, "ui.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)};
	...
	void UI::draw(VkCommandBuffer cmdBuffer)
	{
		...
		if (!imDrawData || imDrawData->CmdListsCount == 0) {
			// No ImGui draw data available, skip rendering
			return;
		}
	...
	bool UI::menuItem(const char *label, const char *shortcut, bool selected)
	{
