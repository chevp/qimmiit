...
namespace oceanbase
{
	VkResult VulkanExampleBase::createInstance(bool enableValidation)
	{
		...
		if (settings.validation)
		{
			...
			ocean::info("Enabling validation layer: VK_LAYER_KHRONOS_validation");
		}
	...
	void VulkanExampleBase::prepare()
	{
		...
		*/
		...
		ocean::info("Command pool queue family index: " + std::to_string(swapChain.queueNodeIndex));
		...
		*/
		...
		/*
			Pipeline cache
		*/
		ocean::info("Creating pipeline cache...");
		auto pipelineCacheStart = std::chrono::high_resolution_clock::now();
		VkPipelineCacheCreateInfo pipelineCacheCreateInfo{};
		pipelineCacheCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
		VK_CHECK_RESULT(vkCreatePipelineCache(device, &pipelineCacheCreateInfo, nullptr, &pipelineCache));
		auto pipelineCacheTime = std::chrono::duration_cast<std::chrono::microseconds>(
			std::chrono::high_resolution_clock::now() - pipelineCacheStart);
		ocean::info("Pipeline cache creation took " + std::to_string(pipelineCacheTime.count() / 1000.0) + " ms");
	...
	VulkanExampleBase::VulkanExampleBase()
	{
		...
		if (!libLoaded) {
			ocean::error("CRITICAL: Failed to load Vulkan library on Android!");
			throw std::runtime_error("Failed to load Vulkan library on Android");
		}
#elif defined(_DIRECT2DISPLAY)
...
#if defined(_WIN32)
		// Only allocate console for GUI applications (elyrion.coregfx.renderer)
		// Skip for console applications (data-driven-coregfx-renderer)
		// Try to attach to parent console first - will fail if already attached
		BOOL attached = AttachConsole(ATTACH_PARENT_PROCESS);
		if (!attached && GetLastError() == ERROR_ACCESS_DENIED) {
			// ERROR_ACCESS_DENIED means console already attached (console application)
			// Skip console allocation to avoid freopen_s() hang
		} else if (!attached) {
			// No console at all - allocate a new one for GUI application
			AllocConsole();
			FILE *stream;
			...
			// Successfully attached to parent console - detach it since we don't need it
			FreeConsole();
		}
#endif
	...
	void VulkanExampleBase::init(const oceancore::Settings *_settings)
	{
		...
		*/
		...
		if (err)
		{
			ocean::error("Could not create Vulkan instance! Error code: " + std::to_string(err));
			throw std::runtime_error("Could not create Vulkan instance! Error code: " + std::to_string(err));
		}
		...
		*/
		...
		if (gpuCount == 0) {
			ocean::error("No Vulkan capable physical devices found!");
		...
		if (err)
		{
			ocean::error("Could not enumerate physical devices! Error code: " + std::to_string(err));
			throw std::runtime_error("Could not enumerate physical devices! Error code: " + std::to_string(err));
		}
			...
			if ((args[i] == std::string("-g")) || (args[i] == std::string("--gpu")))
			{
				...
				if (endptr != args[i + 1])
				{
					...
					else
					{
						...
						vkGetPhysicalDeviceProperties(physicalDevices[index], &deviceProps);
						ocean::info("Selected Vulkan device " + std::to_string(index) + " (" + std::string(deviceProps.deviceName) + ")");
		...
		// select the best GPU
		if (settings.useDiscreteGpu)
		{
			selectedDevice = getIndexOfBestGraphicalUnit(physicalDevices);
		}
		...
		*/
		...
		if (res != VK_SUCCESS)
		{
			ocean::error("Could not create Vulkan device! Error code: " + std::to_string(res));
			throw std::runtime_error("Could not create Vulkan device! Error code: " + std::to_string(res));
		}
		...
		*/
		...
		if (!validDepthFormat) {
			ocean::error("No valid depth format found!");
	...
	HWND VulkanExampleBase::setupWindow(HINSTANCE hinstance, WNDPROC wndproc)
	{
		...
		// wc.lpszClassName = L"OceanRendererClient.IPCOE2"; // Not working: CA2CT(name.c_str());
		...
		if (settings.fullscreen)
		{
			...
			if ((width != (uint32_t)screenWidth) && (height != (uint32_t)screenHeight))
			{
				...
				{
					std::cout << "Fullscreen Mode not supported! Switching to window mode.\n";
		...
		window = CreateWindowExW(WS_EX_ACCEPTFILES,
								sw, // L"OceanRendererClient.IPCOE2",
								sw, // L"OceanRendererClient.IPCOE2",
	...
	void VulkanExampleBase::handleAppCommand(android_app *app, int32_t cmd)
	{
		if (app->userData == NULL) {
			ocean::error("Android app userData is NULL!");
			return;
		}
		VulkanExampleBase *vulkanExample = reinterpret_cast<VulkanExampleBase *>(app->userData);
		switch (cmd)
		...
		case APP_CMD_INIT_WINDOW:
			...
			if (androidApp->window != NULL)
			{
				...
				if (!vulkanExample->prepared) {
					ocean::error("CRITICAL: Vulkan example preparation failed!");
					throw std::runtime_error("Vulkan example preparation failed");
				}
			}
			else
			{
	...
	void VulkanExampleBase::initWaylandConnection()
	{
		...
		if (!display)
		{
			ocean::error("Could not connect to Wayland display!");
			fflush(stdout);
			exit(1);
		}
	...
	void VulkanExampleBase::initxcbConnection()
	{
		...
		if (connection == NULL)
		{
			ocean::error("Could not find a compatible Vulkan ICD!");
			fflush(stdout);
			exit(1);
		}
	...
	void VulkanExampleBase::windowWillResize(float x, float y)
	{
		...
		ocean::info("Window resize initiated");
	}

	void VulkanExampleBase::windowDidResize()
	{
		ocean::info("Window resize completed");
		resizing = false;
	}
#endif
	...
	void VulkanExampleBase::setupFrameBuffer()
	{
		...
		*/
		if (settings.multiSampling)
		{
			...
			VK_CHECK_RESULT(vkBindImageMemory(device, multisampleTarget.color.image, multisampleTarget.color.memory, 0));
	...
	void VulkanExampleBase::setTitle(const std::string& newTitle)
	{
		title = newTitle;
		...
		if (window) {
			...
			int wideLength = MultiByteToWideChar(CP_UTF8, 0, title.c_str(), -1, NULL, 0);
			if (wideLength > 0) {
				std::wstring wideTitle(wideLength - 1, 0);
