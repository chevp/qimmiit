...
namespace oceanutil
{
	VkPipelineShaderStageCreateInfo loadShader(VkDevice device, const std::string_view filename, VkShaderStageFlagBits stage)
	{
		...
		delete[] shaderCode;
#else
		// std::ifstream is("resources/shaders2/glsl/" + filename, std::ios::binary | std::ios::in | std::ios::ate);
		std::string shaderPath = ConfigProvider::getShaderDir() + "/" + std::string(filename);
		std::ifstream is(shaderPath, std::ios::binary | std::ios::in | std::ios::ate);
		...
		{
			std::string errorMsg = "Could not open shader file \"" + std::string(filename) + "\" at path: " + shaderPath;
			std::cerr << "Error: " << errorMsg << std::endl;
			std::cerr << "Please compile shaders first. Run: assets/vkpbr5/shaders/compile.bat" << std::endl;
			shaderStage.module = VK_NULL_HANDLE;
			// Throw a descriptive runtime error instead of assert
			throw std::runtime_error(errorMsg);
		}

#endif
		// Verify shader module was created successfully
		if (shaderStage.module == VK_NULL_HANDLE) {
			throw std::runtime_error("Shader module is null: " + std::string(filename));
		}
		return shaderStage;
	}
	...
	void readDirectory(const std::string_view &directory, const std::string_view &pattern, std::map<std::string, std::string> &filelist, bool recursive)
	{
		...
		std::wstring a = oceanutil::to_wstring(directory) + oceanutil::to_wstring("/") + oceanutil::to_wstring(pattern);
		std::wstring searchpattern(a);
		WIN32_FIND_DATAW data;
		HANDLE hFind;
		if ((hFind = FindFirstFileW(searchpattern.c_str(), &data)) != INVALID_HANDLE_VALUE)
		{
			do
			{
				std::wstring filename(data.cFileName);
				filename.erase(filename.find_last_of(L"."), std::wstring::npos);
				std::string sfilename(filename.begin(), filename.end());
				std::wstring cfilename(data.cFileName);
				std::string scfilename(cfilename.begin(), cfilename.end());
				filelist[sfilename] = std::string(directory) + "/" + scfilename; // data.cFileName;
		...
		if (recursive)
		{
			std::wstring dirpattern = oceanutil::to_wstring(directory) + L"/*";
			if ((hFind = FindFirstFileW(dirpattern.c_str(), &data)) != INVALID_HANDLE_VALUE)
			{
				do
				{
					if (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
					{
						...
						if ((strcmp(data.cFileName, L".") != 0) && (strcmp(data.cFileName, L"..") != 0)) {
