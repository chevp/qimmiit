...
namespace oceanswapchain
{
...
#endif
	{
		...
		{
			std::string errorMsg = "Could not find a graphics and/or presenting queue!";
		...
		{
			std::string errorMsg = "Separate graphics and presenting queues are not supported yet!";
			std::cerr << errorMsg << std::endl;
			ocean::error("CRITICAL: " + errorMsg);
			throw std::runtime_error(errorMsg);
		}
	...
	void VulkanSwapChain::createDirect2DisplaySurface(uint32_t width, uint32_t height)
	{
		...
		for (size_t i = 0; i < displayPropertyCount; ++i)
		{
		...
		for (size_t i = 0; i < planePropertyCount; i++)
		{
